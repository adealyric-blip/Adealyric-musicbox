...
export type Department = 'Women' | 'Unisex' | 'Accessories' | 'Beauty' | 'Bags';
...
const SIZE_CODES = ['2XS', 'XS', 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL', '5XL', '6XL', '7XL', '8XL'];

function parseSizes(s: string): string[] {
  if (!s || s === '—' || s === '-') return [];
  s = s.trim();
  if (s.toLowerCase() === 'one size') return ['One Size'];
  if (s.toLowerCase().startsWith('one size')) return [s];
...
function parseBadges(b: string): string[] {
  if (!b || b === '—' || b === '-') return [];
...
const p = (overrides: Partial<ShopCatalogProduct> & { name: string; department: Department; category: string; price: number }): ShopCatalogProduct => {
  _counter++;
...
// [name, sku, sizes, colorCount, fabric, price, badge]
type ApparelRow = [string, string, string, number, string, number, string];

function apparel(dept: Department, cat: string, sub: string | undefined, rows: ApparelRow[]): ShopCatalogProduct[] {
  return rows.map(([name, sku, sizes, cc, fabric, price, badge], i) =>
    p({
      name,
      sku: sku || undefined,
      sizes,
...
function activewearSub(name: string): string {
...
function onePieceCat(name: string): { category: string; subcategory?: string } {
  if (/Bodysuit/i.test(name)) return { category: 'Bodysuits & Jumpsuits', subcategory: 'Bodysuits' };
  if (/Jumpsuit/i.test(name)) return { category: 'Bodysuits & Jumpsuits', subcategory: 'Jumpsuits' };
  if (/Romper/i.test(name)) return { category: 'Bodysuits & Jumpsuits', subcategory: 'Rompers' };
...
const a1Tshirts = apparel('Women', 'T-Shirts', undefined, [
  ...
  ["Women's One-Shoulder Crop T-Shirt","","S–XL",6,"250gsm/7.4oz",0,""],
  ...
  ["Women's Paneled Crop T-Shirt","","S–XL",4,"260gsm/7.7oz",0,""],
  ...
  ["Women's Color Block Ruched T-Shirt","","S–XL",3,"210gsm/6.2oz",0,""],
  ...
  ["Modal Bodycon Henley Shirt","","4–12",3,"260gsm/7.7oz",0,""],
...
const a5Activewear: ShopCatalogProduct[] = [
  ...
  ["Women's Color Block Yoga Tank Top","","2XS–XL",14,"220gsm/6.5oz",0,"NEW"],
  ...
  ["Women's Cropped Sports Bra","","S–XL",7,"230gsm/6.8oz",0,"BESTSELLER"],
  ...
  ["Women's Pleated Halter Sports Bra","","S–2XL",12,"220gsm/6.5oz",0,"BESTSELLER"],
  ...
  ["Women's Tight Long Sleeve Yoga Bodysuit","","S–2XL",4,"220gsm/6.5oz",0,"NEW"],
  ...
  ["Women's Tight Cropped Sports Bra","","4–12",4,"220gsm/6.5oz",0,"NEW"],
  ...
  p({
    ...
    sizeList: parseSizes(sizes as string),
    colorCount: cc as number,
    fabric: fabric as string,
    price: price as number,
    badges: parseBadges(badge as string),
    department: 'Women',
    category: 'Activewear',
    subcategory: activewearSub(name as string),
    sortOrder: i,
  })
);
...
const a6Shorts = apparel('Women', 'Bottoms', 'Shorts', [
  ...
  ["Snow Washed Raw Edge Drawstring Shorts","","S–XL",2,"300gsm/8.8oz",0,""],
...
const a8Skirts = apparel('Women', 'Dresses', 'Skirts', [
  ...
  ["Women's Studded Denim Mini Skirt","","S–2XL",1,"390gsm/11.5oz",0,"NEW"],
...
const a10OnePieces: ShopCatalogProduct[] = [
  ...
  const cat = onePieceCat(name as string);
  return p({
    name: name as string,
    sku: (sku as string) || undefined,
    sizes: sizes as string,
...
const a11Coats = apparel('Women', 'Outerwear', undefined, [
  ["Women's Mock Neck Zip Up Cardigan","","S–XL",4,"420gsm/12.4oz",0,"NEW"],
...
// ── PART B: Women's Apparel (no SKU codes, 227 rows) ─────────────────────

// [name, sizes, price, badge]
type SimpleApparelRow = [string, string, number, string];

function extractProcessTags(name: string): string[] {
  const tags: string[] = [];
  const patterns: [RegExp, string][] = [
    [/\(All-Over Printing\)/i, 'All-Over Printing'],
    [/\(All-over Printing\)/i, 'All-Over Printing'],
    [/\(Front Printing\)|Front Prints?\b/i, 'Front Printing'],
    [/\(Dual-Sided Printing\)|Dual-Sided Prints?|Dual-sided/i, 'Dual-Sided Printing'],
    [/\(Back Printing\)|Back Printed|Back Print/i, 'Back Printing'],
    [/\bDTG\b/i, 'DTG'],
    [/\bDTF\b/i, 'DTF'],
    [/\(Top Printed\)/i, 'Top Printed'],
    [/\(Back Printed\)/i, 'Back Printing'],
    [/\(Dual-sided Printing\)/i, 'Dual-Sided Printing'],
  ];
...
function categorizePartB(name: string): { category: string; subcategory?: string } {
  if (/Swimsuit|Bikini|Tankini|Swim\b|Cover-?up|Swimwear/.test(name)) return { category: 'Swimwear' };
  if (/Yoga|Leggings|Sports Bra|Sports Tank/.test(name)) return { category: 'Activewear' };
  if (/\bHoodie\b/.test(name)) return { category: 'Hoodies' };
  if (/\bSweatshirt\b/.test(name)) return { category: 'Sweatshirts' };
  if (/\bJacket\b|\bOuterwear\b/.test(name)) return { category: 'Outerwear' };
  if (/\bVest\b|\bCardigan\b/.test(name)) return { category: 'Outerwear' };
  if (/\bJumpsuit\b|\bBodysuit\b/.test(name)) return { category: 'Bodysuits & Jumpsuits', subcategory: 'Jumpsuits' };
  if (/Pajama|Nightdress|Nightgown|Loungewear|Robe|Sleep/.test(name)) return { category: 'Lingerie & Sleep' };
  if (/Underwear|Thong|Panties|Briefs|G-?String|Lingerie|Tanga/.test(name)) return { category: 'Lingerie & Sleep', subcategory: 'Underwear' };
  if (/\bDress\b/.test(name)) return { category: 'Dresses' };
  if (/\bPants\b|\bJogger\b|\bSweatpant/.test(name)) return { category: 'Bottoms' };
  if (/\bShorts?\b/.test(name)) return { category: 'Bottoms', subcategory: 'Shorts' };
  if (/\bSkirt\b|\bSkort\b/.test(name)) return { category: 'Dresses', subcategory: 'Skirts' };
  if (/\bTank\b|Camisole|\bCami\b/.test(name)) return { category: 'T-Shirts', subcategory: 'Tank Tops & Camis' };
  if (/\bCrop\b/.test(name)) return { category: 'T-Shirts', subcategory: 'Crop Tops' };
  if (/Polo|POLO/i.test(name)) return { category: 'T-Shirts', subcategory: 'Polo Shirts' };
  if (/\bSet\b|Cheerleader/.test(name)) return { category: 'Bodysuits & Jumpsuits', subcategory: 'Rompers' };
  if (/\bSweater\b|Pullover/.test(name)) return { category: 'Sweatshirts' };
  if (/T-?Shirt|Tee|Shirt|Blouse|Jersey|Top/.test(name)) return { category: 'T-Shirts' };
  return { category: 'T-Shirts' };
}

const partBRaw: SimpleApparelRow[] = [
  ["Women's shirts (All-Over Printing)","S–3XL",9.28,""],
  ["Best Hoodies-Double-Layer Cap Women's Hoodies","S–6XL",9.92,""],
  ["Loose High Waisted Shorts (All-Over Printing)","S–3XL",7.46,""],
  ["Women Top T-Shirts (All-Over Printing)","S–6XL",5.74,""],
  ["DTG Personalized T-Shirts-Dual-Sided Printing","S–3XL",5.91,""],
  ["Sports Bra Designs-Yoga Support Bras (All-Over Printing)","S–2XL",7.14,""],
  ["Honeycomb Textured Yoga Pants for Women","S–2XL",10.14,""],
  ["Crewneck T-Shirts (All-Over Printing)","S–5XL",5.76,""],
  ["Mom Shirts (All-Over Printing)","XS–5XL",6.39,""],
  ["Swimsuit Cover-up (All-Over Printing)","One size",13.63,""],
  ["Baggy Pants for Women (All-Over Printing)","S–2XL",7.03,""],
  ["Sleeveless Ankle-length Dress (All-Over Printing)","S–5XL",10.24,""],
  ["V-Neck Baggy Dress with Pocket","S–5XL",8.31,""],
  ["Summer Sweet Bowknot Dress (All-Over Printing)","2XS–6XL",12.77,""],
  ["Designer T-Shirts for Women-Front Prints","S–2XL",6.98,""],
  ["Designed Women's Hoodies Front Prints","S–3XL",10.03,""],
  ["Hoodies for Women Dual-Sided Prints","S–3XL",12.17,""],
  ["T-Shirts for Women-Printed Tees (Front Printing)","L–3XL",4.91,""],
  ["Patterned Ribbed Yoga Sets (All-Over Printing)","S–2XL",8.53,""],
  ["DTF Designed Women's T-Shirts-Dual-Sided Printing","S–2XL",8.14,""],
  ["Women's Tank Top (All-Over Printing)","S–2XL",5.10,""],
  ["Yoga Pants with 2 Pockets (All-Over Printing)","S–2XL",11.31,""],
  ["Yoga Tank Tops (All-Over Printing)","S–2XL",7.24,""],
  ["Yoga Workout Outfits (All-Over Printing)","XS–2XL",10.69,""],
  ["Women's Long Sleeve U-Neck T-Shirts (All-Over Printing)","S–5XL",6.34,""],
  ["Round Neck Sleeveless Dress Styles","XS–5XL",7.03,""],
  ["Cover-up Dress for Swimwear (All-Over Printing)","S–2XL",12.45,""],
  ["Women's Halter One-Piece Swimsuits (All-Over Printing)","S–2XL",11.31,""],
  ["Sleeveless Dress","S–5XL",6.39,""],
  ["Ankle-length Slip Dress (All-Over Printing)","S–5XL",10.24,""],
  ["Beach Wrap Skirt (All-Over Printing)","S–2XL",4.03,""],
  ["Halter Tie Front Bikini Top (All-Over Printing)","S–2XL",5.76,""],
  ["Halter Tie Front Bikini Set (All-Over Printing)","S–2XL",8.53,""],
  ["Two Piece Halter Swimsuit (All-Over Printing)","XS–2XL",8.53,""],
  ["Halter Triangle Bikini (All-Over Printing)","XS–2XL",9.39,""],
  ["2-Piece Women's Halter Tankini (All-Over Printing)","XS–XL",11.96,""],
  ["Tankini Two Piece Swimsuit (Top Printed)","S–3XL",12.60,""],
  ["Sweatshirt Designs Raglan Style","S–3XL",7.84,""],
  ["Sports Bra Designs-Yoga Support Bras","S–2XL",7.14,""],
  ["Patterned Yoga Sports Bra","S–2XL",7.14,""],
  ["Dual-Sided Printed T-Shirts for Women","S–2XL",9.43,""],
  ["Print Crew Neck T-Shirts for Women (Front Printing)","S–3XL",4.31,""],
  ["St Patricks Day Shirts-Short Sleeve Raglan Tees","S–3XL",6.60,""],
  ["Long Sleeve T-Shirts with Pocket","2XS–6XL",7.24,""],
  ["Women's T-Shirts Front Prints","S–2XL",7.50,""],
  ["Spaghetti Strap Jumpsuit (All-Over Printing)","S–3XL",11.49,""],
  ["Tank Top Bodysuit (All-Over Printing)","S–2XL",8.08,""],
  ["Women's 2-Piece Pajama Set (All-Over Printing)","XS–6XL",11.74,""],
  ["Women's Pocket Hoodie Back Printed","S–3XL",10.24,""],
  ["Women Long Sleeve Turtleneck Sweater (All-Over Printing)","S–2XL",11.91,""],
  ["Workout Yoga Set","S–2XL",8.70,""],
  ["Suspender Jumpsuit (All-Over Printing)","S–3XL",11.10,""],
  ["Women's Home Pajamas Pants (All-Over Printing)","S–5XL",7.56,""],
  ["V-Neck Women Night Wear Suit (All-Over Printing)","S–XL",13.20,""],
  ["Wearable Sweater Oversize","One size",12.81,""],
  ["Long Sleeve Nightdress (All-Over Printing)","S–XL",7.41,""],
  ["Ruffle V-Neck T-Shirt (All-Over Printing)","S–5XL",5.91,""],
  ["Long Sleeve Drop Shoulder Open Shirt (All-Over Printing)","S–2XL",9.75,""],
  ["Tanga for Women (All-Over Printing)","S–2XL",3.77,""],
  ["2-Piece Ruffle Tankini Swimsuit (All-Over Printing)","S–2XL",12.56,""],
  ["Print Patterned Yoga Pants with 2 Pockets","S–2XL",8.70,""],
  ["Short Sleeve Loungewear Set (All-Over Printing)","S–5XL",9.77,""],
  ["Spaghetti Dress (All-Over Printing)","S–2XL",9.60,""],
  ["Personalized Drawstring Yoga Pants (Straight Baggy)","S–3XL",6.34,""],
  ["Women's Bikini Bottom (All-Over Printing)","S–2XL",5.12,""],
  ["Raglan Women's 3/4 Sleeve T-shirt","S–3XL",6.81,""],
  ["Raglan Women's Long Sleeve T-shirt","S–3XL",7.24,""],
  ["Women's Printed Sweatshirt (Front Printing)","S–3XL",8.96,""],
  ["Drop shoulder Hoodie (All-Over Printing)","S–4XL",7.24,""],
  ["1/4 Zipper Long Sleeve Zip Gym Top (All-Over Printing)","S–3XL",9.92,""],
  ["Cotton Women's Back Print Hoodie","S–3XL",10.03,""],
  ["Printed V-neck Cami Top (All-Over Printing)","S–2XL",7.61,""],
  ["Short Tank Top (All-Over Printing)","S–2XL",5.96,""],
  ["Floral Camisole","S–2XL",4.89,""],
  ["Sleeveless Crop Top (All-Over Printing)","S–2XL",6.19,""],
  ["Women's Zipper Jacket (All-Over Printing)","XS–XL",8.31,""],
  ["Ladies Swim Shorts","S–2XL",4.91,""],
  ["Sports Yoga Bra (All-Over Printing)","S–2XL",6.69,""],
  ["Bell Bottoms (All-Over Printing)","XS–XL",5.12,""],
  ["Women's Jogger Sweatpants (All-Over Printing)","S–3XL",7.26,""],
  ["Diagonal Shoulder Two Piece Set (All-Over Printing)","S–5XL",10.03,""],
  ["Hot Yoga Pants for Women (All-Over Printing)","S–2XL",10.03,""],
  ["Women Yoga Shorts (All-Over Printing)","S–2XL",6.60,""],
  ["Women's Two Piece Swimsuit High Neck Halter Top","S–2XL",9.62,""],
  ["High-collar Long-sleeve Bodysuit (All-Over Printing)","S–2XL",10.20,""],
  ["V-neck 3/4 Sleeve Dress with Pockets","S–5XL",8.06,""],
  ["One Shoulder Split Dress (All-Over Printing)","XS–L",8.96,""],
  ["Long Sleeve Hoodie Dress (All-Over Printing)","S–5XL",11.31,""],
  ["Short Sleeve V Neck Lovely Dress (All-Over Printing)","2XS–6XL",10.03,""],
  ["Round Neck Short Sleeve Dress (All-Over Printing)","2XS–8XL",11.06,""],
  ["Thong Panties (All-Over Printing)","XS–XL",4.41,""],
  ["Cutout Bodycon Slim Fit Dress (All-Over Printing)","S–5XL",10.03,""],
  ["Sleeveless Ladies Tank Dress (All-Over Printing)","S–2XL",8.08,""],
  ["Women's Off the Shoulder Half-Sleeve T-shirt","S–5XL",5.96,""],
  ["V Neck Sleeveless Dress (All-Over Printing)","S–2XL",8.06,""],
  ["Women Short Sleeve Shirt (All-Over Printing)","S–4XL",3.60,""],
  ["V-neck Short Sleeve T-Shirt (All-Over Printing)","S–5XL",5.91,""],
  ["V-neck Loose Long Sleeve T-shirt (All-Over Printing)","S–5XL",8.06,""],
  ["Women's Flower Blouse Long Sleeve (All-Over Printing)","S–5XL",8.74,""],
  ["Short Sleeve Navel Shirt Baby Tee (All-Over Printing)","S–3XL",2.57,""],
  ["Women's Irregular Shirt (All-Over Printing)","S–8XL",10.41,""],
  ["Drop Shoulder Women's Open Shirt (Irregular Hem)","S–2XL",11.70,""],
  ["Casual Long Sleeve Vintage Tour Shirt (All-Over Printing)","S–3XL",8.10,""],
  ["Women's Straight POLO Shirt (All-Over Printing)","S–5XL",8.74,""],
  ["Plus Size V-neck Long Sleeve Dress (All-Over Printing)","S–8XL",8.49,""],
  ["V-neck Women Top with Quarter Sleeve (All-Over Printing)","S–5XL",8.53,""],
  ["One-Piece Swimsuit (All-Over Printing)","S–2XL",10.46,""],
  ["Spaghetti Strap Dress for Women (All-Over Printing)","S–5XL",11.06,""],
  ["Yoga Knickerbockers (All-Over Printing)","S–2XL",7.35,""],
  ["Cotton Printed Hoodie for Women (Front Printing)","S–3XL",10.03,""],
  ["Zipper T-Shirts","S–3XL",7.61,""],
  ["Women's Graphic Hoodie (All-Over Printing)","S–3XL",6.84,""],
  ["Cotton Women's Size Hoodie with Pocket","S–3XL",10.24,""],
  ["Full Zipper Long Sleeve Hoodie (All-Over Printing)","S–5XL",13.63,""],
  ["Hooded Pullover (All-Over Printing)","S–5XL",11.10,""],
  ["Poncho-style Long Sleeve Hoodie (Irregular Hem)","S–5XL",12.77,""],
  ["Cotton Women's Hoodie with Pocket (Dual-sided)","S–3XL",12.17,""],
  ["Lady Hoodie with Double-layer Cap","S–5XL",7.05,""],
  ["Hoodie for Women with Double-layer Cap","S–6XL",7.63,""],
  ["Lightweight Women's Hoodie (All-Over Printing)","S–5XL",4.84,""],
  ["Turtle Neck Long Dress for Women (All-Over Printing)","XS–XL",11.91,""],
  ["Loose Long-sleeve Dress with Pockets","S–5XL",8.06,""],
  ["Sleeveless High Waist Dress (All-Over Printing)","S–2XL",13.80,""],
  ["Long Dress Set (All-Over Printing)","S–5XL",9.81,""],
  ["Plus Size Halter Tie Front Bikini Set","XL–4XL",9.17,""],
  ["Plus Size Two Piece Swimsuit High Neck Halter Top","XL–4XL",10.26,""],
  ["Multi-Strap Lace Briefs (All-Over Printing)","S–2XL",5.31,""],
  ["Sexy One Piece Women Lingerie Bodysuit Deep V Teddy","S–3XL",6.34,""],
  ["Faux Cotton Women Zip up Jumpsuit Pajama","S–4XL",13.67,""],
  ["Women Flannel Zip up Jumpsuit Pajama","S–4XL",14.31,""],
  ["Women's T-shirt with Short Sleeves","S–3XL",4.20,""],
  ["Sexy G String Thong for Women","S–3XL",4.63,""],
  ["Sexy G String Thong for Women","S–3XL",6.39,""],
  ["Anti-peep Skort with Pocket","S–4XL",5.49,""],
  ["Women Slim Fit Slip Dress","S–2XL",7.67,""],
  ["Faux Cotton Drawstring Wide Leg Pajama Pants","S–5XL",6.99,""],
  ["Women Washed Vintage T-Shirt (Front Printing)","S–2XL",6.53,""],
  ["Women Washed Vintage T-Shirt (Front Printing)","S–2XL",8.33,"Premium"],
  ["Round Neck Long Sleeve Dress with Pocket","S–2XL",9.81,""],
  ["Sleeveless Vest (Dual-sided Printing)","S–2XL",11.12,""],
  ["Long Sleeve Dress (All-Over Printing)","S–2XL",7.41,""],
  ["Workout Yoga Set (All-over Printing)","S–2XL",11.31,""],
  ["Workout Sports Bra for Women","S–2XL",4.97,""],
  ["Knee-Length Yoga Shorts (All-over Printing)","S–2XL",9.17,""],
  ["Faux Cotton Short Sleeve T-Shirt","S–5XL",5.23,""],
  ["Women Workout Yoga Pants with Lines","S–2XL",8.74,""],
  ["Spaghetti Strap Dress (All-Over Printing)","S–2XL",12.45,""],
  ["2-Piece Women's Tankini Swimsuit (Top Printed)","S–6XL",13.46,""],
  ["Women's Long Sleeve Sun Protection Shirt-UPF 40 UV Rash Guard","S–2XL",6.99,""],
  ["2-Piece Women's Tankini Set High Neck Halter Top","XS–2XL",9.81,""],
  ["Plus Size Women's Halter Bikini Set","XL–4XL",9.17,""],
  ["Women's One Piece Tummy Control V Neck Swimwear","S–2XL",10.69,""],
  ["Plus Size Women Tankini 2-Piece Swimsuit (Top Printed)","XL–5XL",12.60,""],
  ["Worn-effect Fabric Women Satin Pajama Set","XS–5XL",11.96,""],
  ["Cooling Polyester Ruffle V-Neck T-Shirt","S–5XL",6.24,"Premium"],
  ["Cooling Short Sleeve Loungewear Set","S–5XL",10.84,"Premium"],
  ["Ice Silk V Neck Short-Sleeve Women Shirt","S–5XL",5.10,""],
  ["Ice Silk Women Underwear Thongs","S–2XL",4.03,""],
  ["2-Piece Bikini Swimwear (All-Over Printing)","S–2XL",6.39,""],
  ["2-Piece Thong Triangle Bikini Set","XS–2XL",7.46,""],
  ["Strapless Women Tube Top Y2K Bandeau","S–2XL",4.03,""],
  ["Swimming Cap for Women and Men","One size",3.17,""],
  ["Plus Size Two Piece Swimsuit with Tankini","XL–4XL",15.60,""],
  ["Plus Size Round-neck Short Sleeve Dress","L–4XL",8.27,""],
  ["Asymmetrical Hem Shirt (All-Over Printing)","S–8XL",11.27,"Premium"],
  ["Polar Fleece Double-Zip Jacket (All-Over Printing)","S–6XL",22.67,"Premium"],
  ["Long-Sleeve Raglan T-Shirt for Women","S–6XL",9.84,"Premium"],
  ["Loose-Fit 3/4 Sleeve Boat Neck T-Shirt","S–5XL",7.67,"Premium"],
  ["Round Neck 3/4 Sleeve Long Dress","S–8XL",10.46,"Premium"],
  ["Wide Round Neck 3/4 Sleeve Long Dress","S–8XL",13.46,"Premium"],
  ["Sleeveless Ankle-length Dress","S–5XL",10.67,"Premium"],
  ["Short Sleeve V Neck Lovely Dress","2XS–6XL",10.78,"Premium"],
  ["Tank Top Pocket Dress","2XS–5XL",6.77,""],
  ["Slim Fit Spaghetti Strap Dress for Women","XS–6XL",5.31,""],
  ["Sleeveless Dress for Women","S–3XL",5.10,""],
  ["Cheerleader Set for Women","S–4XL",8.27,""],
  ["Cheerleader Dress for Women","XS–2XL",5.74,""],
  ["Women Baseball Jersey","S–4XL",7.67,""],
  ["Cotton Summer Short Sleeve T-Shirts for Women","2XS–4XL",9.99,""],
  ["Cotton Pajama Pants for Women","S–5XL",11.27,""],
  ["Cotton Thong Underwear for Women","S–2XL",5.49,""],
  ["Button-Down Polo Shirt for Women","S–3XL",6.71,""],
  ["Workout Crop Yoga Tank Top","S–2XL",8.53,""],
  ["Slit Fitted Camisole Cheongsam","S–4XL",7.78,""],
  ["One Size Ice Silk Nightdress","XL",6.39,""],
  ["Women's Raglan T-shirt","S–5XL",5.27,"Premium"],
  ["Women's Baseball Jacket","XS–5XL",9.34,""],
  ["Women's Ice Silk Top with Half-Sleeve","S–6XL",6.17,"Premium"],
  ["Women's Cool Touch Fitted Quick Dry Top","S–6XL",10.89,"Premium"],
  ["Women's Long Sleeve Quick Dry Top","S–6XL",8.31,"Premium"],
  ["Women's Long Sleeve Sweater Dress","S–2XL",8.91,""],
  ["V-neck Dress for Women","S–3XL",7.41,""],
  ["Women's Sleeveless Polo Shirt","XS–4XL",5.53,""],
  ["Women's Drop Shoulder Cardigan","S–3XL",8.96,""],
  ["Women's Stand-Up Collar Fleece Jacket","S–4XL",9.56,""],
  ["Women's Boxer Briefs","S–3XL",4.20,""],
  ["Stand-up Collar Fleece Casual Jacket","S–6XL",27.39,"Premium"],
  ["Winter Quilted Long-Sleeve Hooded Outerwear","S–6XL",37.24,"Premium"],
  ["Fleece Sweatshirt Short Jacket for Women","S–6XL",13.46,"Premium"],
  ["Double-Layer Hooded Top","S–6XL",23.31,"Premium"],
  ["Full-Edge Binding Slim-Fit Jacket","S–6XL",16.46,"Premium"],
  ["Double-Layered Hoodie with Pockets","S–6XL",16.99,"Premium"],
  ["Adult Hooded Sweatshirt","One size",11.53,""],
  ["260gsm Women's Cute Sweatshirt (Back Printing)","S–3XL",8.96,""],
  ["Collarless Dress","XS–5XL",7.84,""],
  ["Women's V Neck Long Sleeved Top","S–5XL",6.60,""],
  ["Bikini Swimsuit (All-Over Printing)","XS–2XL",8.64,""],
  ["Women's High-Rise Panties","S–2XL",3.60,""],
  ["Straight-Leg Drawstring Yoga Pants","S–2XL",7.20,""],
  ["Women's Ruffled Sweatshirt","S–5XL",6.34,""],
  ["Women's Low-Rise Panties","S–2XL",3.24,""],
  ["Women's Wide Leg Pant","XS–3XL",7.20,""],
  ["Women's Drop Shoulder Shirt","S–3XL",8.06,""],
  ["Women's V Neck Ruffled Sleeve Top","S–3XL",6.34,""],
  ["Lace Thong","XS–3XL",3.81,""],
  ["Mediterranean Style V Neck Mid-Sleeve Robe","S–5XL",10.20,""],
  ["Mediterranean Style V-Neck Suit","XS–5XL",11.10,""],
  ["Women's Mediterranean Style Set","XS–5XL",15.13,""],
  ["Midi Spaghetti Strap Dress","XS–5XL",6.60,""],
  ["Lace-Trimmed Camisole Nightgown","S–4XL",6.28,""],
  ["Ice Silk Camisole Pajama Set","XS–4XL",10.89,""],
  ["Ruffled Short-Sleeve Pajama Set","S–5XL",9.60,""],
  ["Women's V-Neck Pajama Set","S–5XL",11.96,""],
  ["V-Neck Short Sleeve Pajama Set","S–5XL",9.99,""],
  ["Women's 3/4 Sleeve Top","S–3XL",7.67,""],
  ["Side Slit Dress","S–5XL",9.39,""],
  ["V-Neck Short Sleeve Top","2XS–5XL",5.31,""],
  ["Women's Yoga Sports Tank","S–6XL",9.60,"Premium"],
  ["Short Sleeve Collared Shirt Dress","XS–6XL",9.77,""],
  ["One Piece Swimsuit for Women","S–2XL",10.89,""],
  ["Women's Loose Fit Casual Pants","S–5XL",8.91,""],
  ["Off the Shoulder Top","S–2XL",4.63,""],
  ["Women's Lace Panties","XS–3XL",3.81,""],
  ["Women's V Neck Short Sleeve T-Shirt","S–2XL",4.41,""],
  ["Women's Backless Top","XS–L",4.67,""],
  ["Women's Loungewear Pajama Set","S–5XL",12.77,""],
  ["Button-Up Cardigan","S–5XL",11.96,""],
  ["Long Sleeve Mesh T-Shirt","S–5XL",5.42,""],
  ["Lace Trim V-Neck 3/4 Sleeve T-Shirt","S–5XL",7.48,""],
  ["Chic Strapless Bandana Style Bikini","XS–2XL",8.10,""],
  ["Women's V Neck Long Sleeve Hoodie","S–3XL",7.20,""],
  ["Sleeveless Cardigan Vest","S–2XL",7.63,""],
  ["Cropped T-Shirt","S–3XL",5.70,"Premium"],
  ["Women's Casual Blazer","S–7XL",9.77,""],
  ["Flannel Women's Sleep Pants","S–5XL",7.89,""],
  ["Sheer Mesh One-Piece Swimsuit","S–2XL",11.31,""],
  ["One Size Flannel Nightdress","XL",7.63,""],
  ["V Neck Tank Top","S–3XL",5.06,""],
  ["Puff Short Sleeve Top","2XS–3XL",6.99,""],
  ["Women's Lightweight Cardigan","S–6XL",8.42,""],
  ["Women's Ruffle T-shirt","XS–5XL",6.99,""],
  ["Men's POLO Shirt","S–5XL",10.03,"Premium"],
  ["Women's POLO Shirt","S–5XL",10.03,"Premium"],
  ["Double Layer V-Neck Top","S–5XL",8.27,"Premium"],
  ["Short Sleeve Nightdress","S–3XL",5.53,""],
  ["Sleeveless Knit Sweater","S–3XL",5.10,""],
  ["Ruffle Sleeve Dress","S–5XL",7.41,""],
  ["Women's 2-Button POLO Shirt","S–6XL",8.96,"Premium"],
  ["Short Cami Dress","XS–5XL",6.17,""],
  ["Women's Lace Trim Pajama Shorts","S–5XL",7.20,""],
  ["Button Down Collar Dress","S–6XL",12.41,"Premium"],
  ["Women's Flare Sleeve Tee","XS–5XL",5.10,""],
  ["Lace Trimmed Short Sleeve Nightgown","S–5XL",9.81,""],
  ["Women's Ruffled Sleep Pants","S–5XL",8.06,""],
  ["Women's Ruffled Cover Up Dress","S–2XL",6.49,""],
  ["Women's V Neck Rugby Jersey","XS–5XL",4.89,""],
  ["Ruffled Yoga Skort with Pockets","XS–3XL",6.13,""],
  ["Square Neck One Piece Swimsuit","S–2XL",8.96,""],
  ["Women's Ruffled Sleeve One Piece Swimsuit","S–2XL",9.17,""],
  ["Women's Halter Tie Vest","XS–5XL",3.49,""],
  ["Mesh High Stretch Dress Cover Up","S–5XL",7.74,""],
  ["Button Down Shirts for Women","XS–5XL",7.89,""],
  ["Yoga Jogger Pants","S–2XL",9.13,"Premium"],
  ["Square Neck Tank Top","S–5XL",6.49,""],
  ["Women's POLO Tank Top","XS–4XL",7.29,"Premium"],
  ["Ruffled V Neck Dress","XS–2XL",9.56,""],
  ["Mesh One-Piece Swimsuit","S–2XL",9.13,""],
  ["V Neck Camisole","XS–4XL",5.10,""],
  ["Flared Sleeve T-shirt","S–5XL",5.91,""],
  ["Two-Piece Swimsuit Set","S–2XL",11.31,""],
  ["High Waisted Quick Drying Shorts","S–3XL",8.64,"Premium"],
  ["Women's Cuban Collar Pajama Set","S–2XL",13.20,""],
  ["Women's Short Sleeve Pajama Set","S–4XL",10.41,""],
  ["High Neck Tank Top","S–5XL",6.73,"Premium"],
  ["Women's Casual V Neck Dress","S–5XL",6.81,""],
  ["Flared Slit Dress","S–5XL",8.10,""],
  ["V Neck Sleeveless Dress","XS–5XL",8.10,"Premium"],
  ["Women's Ruffle Long Sleeve Robe","S–5XL",9.34,""],
  ["Short Sleeved Wide Leg Pants Set","S–5XL",8.74,""],
  ["Sleeveless Top and Wide Leg Pants Set","S–5XL",9.71,""],
  ["Strap Top Wide Leg Pants Set","S–5XL",8.74,""],
  ["Lace Trim Pajama Set","XS–2XL",7.41,""],
  ["Ruffled Pajama Set","S–5XL",10.84,""],
  ["Women's Button Up Vest","S–3XL",4.89,""],
  ["High Neck Mesh Top For Women","S–5XL",5.53,""],
  ["Casual V Neck Dress For Women","S–3XL",9.99,""],
  ["Irregular Hemmed Skirt","S–3XL",6.13,""],
  ["Women's High Waisted Pants","S–5XL",9.56,""],
  ["Flared Sleeve Dress For Women","S–5XL",6.99,""],
  ["Mesh One Piece Swimsuit For Women","S–2XL",12.81,""],
  ["Strap One Piece Swimsuit For Women","S–2XL",12.81,""],
  ["Women's V Neck One Piece Swimsuit","S–2XL",12.81,""],
  ["Bat Sleeve Dress For Women","S–5XL",6.99,""],
  ["Women's Bubble Sleeve T-Shirt","S–5XL",6.34,""],
  ["Women's Square Neck Long Sleeved Shirt","XS–5XL",6.34,""],
  ["Off Shoulder Hoodie","S–5XL",6.39,""],
  ["Square Collar Long Sleeved Jumpsuit","XS–5XL",6.99,""],
  ["Women's Button Up Hoodie","S–5XL",7.46,""],
  ["V-Neck Pajama Set","S–3XL",9.34,""],
  ["Netted Swimsuit Set For Women","S–2XL",13.67,""],
  ["Sporty Bikini Swimwear Set","S–2XL",9.39,""],
  ["Mesh Splicing One-Piece Swimsuit","S–2XL",10.03,""],
  ["Backless One-Piece Swimsuit","S–2XL",11.96,""],
  ["Casual Long Sleeve V-Neck Sweatshirt","S–5XL",6.81,""],
  ["Women's Square Neckline Ruffle Dress","S–5XL",8.98,"Premium"],
  ["Women's Suit Vest","S–5XL",5.96,""],
  ["Cotton Women's Short Sleeved Shirt","XS–5XL",11.10,""],
  ["Irregular Shirt For Women","S–3XL",13.46,""],
  ["Button Down Shirt For Men","XS–4XL",11.31,""],
  ["Women's Straight Wide Leg Pants","S–5XL",8.91,""],
  ["Lantern Long Sleeve Shirt","S–5XL",8.96,""],
  ["Women's Long Sleeve Cardigan With Pockets","S–5XL",7.84,""],
  ["Ankle-length Slip Dress With Straps","S–5XL",8.91,""],
  ["Short Sleeved Pajama Set","S–3XL",8.91,""],
];

const partBProducts: ShopCatalogProduct[] = partBRaw.map(([name, sizes, price, badge], i) => {
...
// ── PART C: Beauty (143 rows) ─────────────────────────────────────────────

// [name, tags, size, price, badge]
type BeautyRow = [string, string, string, string, string];

function categorizeBeauty(name: string, tags: string[], badge: string): string {
...
function parseBeautyPrice(raw: string): { price: number; originalPrice?: number } {
...
const partCProducts: ShopCatalogProduct[] = beautyRows.map(([name, rawTags, size, rawPrice, badge], i) => {
...
type BagRow = [string, string, string, string];

const bagRows: BagRow[] = [
...
const partDProducts: ShopCatalogProduct[] = bagRows.map(([name, material, dimensions, badge], i) =>
  p({
    name,
    material,
    dimensions,
...
const ALL_PRODUCTS: ShopCatalogProduct[] = [
  ...a1Tshirts,
  ...a2Tanks,
  ...a3Hoodies,
  ...a4Sweatshirts,
...
const _slugIndex = new Map<string, ShopCatalogProduct>();
const _idIndex = new Map<string, ShopCatalogProduct>();
...
export function getAllProducts(): ShopCatalogProduct[] {
  return ALL_PRODUCTS;
}

export function getProductsByDepartment(dept: Department): ShopCatalogProduct[] {
  return ALL_PRODUCTS.filter(p => p.department === dept);
}

export function getProductsByCategory(dept: Department, category: string): ShopCatalogProduct[] {
  return ALL_PRODUCTS.filter(p => p.department === dept && p.category === category);
}

export function getCategoriesForDepartment(dept: Department): { category: string; subcategories: string[]; count: number }[] {
...
export function getDepartments(): { dept: Department; label: string; count: number }[] {
  ...
  return Object.keys(labels).map(dept => ({
    ...
    label: labels[dept as Department],
    count: deptCounts.get(dept as Department) || 0,
  })).sort((a, b) => b.count - a.count);
}
...
export function getProductBySlug(slug: string): ShopCatalogProduct | undefined {
  return _slugIndex.get(slug);
}

export function getProductById(id: string): ShopCatalogProduct | undefined {
