...
function AdminProductDetailPage({ product }: { product: any }) {
  const router = useRouter();
  const { addToCart } = useAppStore();
  const [selectedSize, setSelectedSize] = useState<string | null>(null);
            ...
            <div className="mt-6 border-t border-black/5 pt-6 space-y-3">
              ...
              {hasColors && (
                <div className="flex justify-between items-center">
                  <span className="text-[11px] uppercase tracking-[0.15em] text-black/40">Available Colors</span>
                  <div className="flex gap-1.5">
                    {product.colors.map((c: { name: string; hex: string }) => (
                      <span key={c.name} className="flex items-center gap-1 text-[10px] text-black">
                        <span className="inline-block w-3 h-3 rounded-full border border-black/10" style={{ backgroundColor: c.hex }} />
                        {c.name}
                      </span>
                    ))}
                  </div>
...
export function ShopDetailRouter({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    fetch(`/api/shop/admin-products/slug?slug=${encodeURIComponent(slug)}`)
      .then((r) => r.json())
      .then((res) => {
  ...
  if (adminLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <div className="animate-pulse text-sm text-black/30">Loading...</div>
      </div>
    );
  }

  if (adminProduct) {
    return <AdminProductDetailPage product={adminProduct} />;
  }
